...
export default function AdminSalariesPage() {
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setForm(json.data);
      setMeta(json.meta);
      alert("Salariés mis à jour.");
    } catch {
      alert("Erreur lors de l'enregistrement des salariés.");
    } finally {
      setSaving(false);
    }
  }
  ...
  function addSalary() {
    setForm((current) => ({
      ...current,
      salaries: [...current.salaries, { ...emptySalary }],
    }));
  }
      ...
      <header className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="space-y-2">
            <h1 className="text-2xl font-bold">Salariés diplômés</h1>
            <p className="max-w-3xl text-sm text-muted-foreground">
              Modifiez ici les salariés affichés sur la page publique
              /club/salaries. Ils sont séparés du comité directeur.
            </p>
          </div>

          <div className="flex flex-wrap gap-2">
            <Button asChild variant="outline">
              <Link href="/admin/comite-directeur">Modifier le comité</Link>
            </Button>
            <Button asChild variant="outline">
        ...
        <Card className="border-primary/20 bg-primary/5">
          <CardHeader className="space-y-2 pb-3">
            <CardTitle className="text-base">Source salariés</CardTitle>
            <CardDescription>
              Les modifications enregistrées ici utilisent la même source de
              données que le comité, mais la page publique est distincte.
            </CardDescription>
          </CardHeader>
      ...
      <form onSubmit={submit} className="space-y-6">
        <Card>
          <CardHeader className="flex flex-row items-start justify-between gap-4 space-y-0">
            <div className="space-y-1.5">
              <CardTitle>Équipe salariée</CardTitle>
              <CardDescription>
                Ajoutez un nom, une description et une photo pour chaque salarié
                ...
                <div className="grid gap-4">
                  ...
                  <div className="grid gap-2">
                    ...
                    <textarea
                      ...
                      onChange={(event) =>
                        updateSalary(index, "description", event.target.value)
                      }
                    />
                  </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Button type="submit" disabled={loading || saving}>
            ...
            {saving ? "Enregistrement..." : "Enregistrer les salariés"}
          </Button>
        </div>
      </form>
    </div>
